...
function request_access_token(pokemondata, res){
    ...
    let payload = `https://discordapp.com/oauth2/authorize?client_id=${ClientID}&scope=identify+guilds&response_type=code&callback_uri=http://localhost:3000/authorize`;
    console.log(payload);
    res.writeHead(302, {'Location': payload});
    res.end();

    
    // const tokenRequest = https.request(formData, options);
